...
- **고르지 않아도 기본값이 있다.** 시나리오·회고 생성은 노션 연결만 요구하는데
  ...
  `defaultDb`로 기본값을 정해 세션에 박는다: 워크스페이스에 이미 있는 stories DB
  (`searchStoriesDbs` — 페이지를 타고 내려가지 않으므로 깊이 정리해 둔 DB도 찾는다),
  없으면 가장 최근에 편집한 공유 페이지 아래에 생성. 기본값조차 못 정하면 `enabled:
  false`와 `error` 이유를 함께 돌려준다. 사용자가 직접 고른 게 아니라는 사실은 세션의
  `autoRoot`로 남고, 그동안은 `chosen: false`라 저장 위치를 한 번 알려준다.
