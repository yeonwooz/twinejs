...
- **스토리별로 저장 위치가 어긋나면 그 줄이 말한다.** `다른 곳에 저장됨`(예전 DB에 있음) /
  `노션에 없음`(올라간 적 없음)이 붙고, 그 줄에만 `여기로 옮기기` 버튼이 뜬다
  (`move-story-button.tsx`). 헤더 표시등은 앱 전체가 저장되는지만 말하므로 스토리 하나가
  뒤처진 건 지금까지 어디에도 안 보였다. 저장 위치를 바꿔도 예전 것은 따라오지 않는데
  (이후 저장분만 새 위치로 간다) 옮길 방법이 "편집기에서 하나씩 건드리기"뿐이었다.
  누르면 디바운스를 건너뛰고 바로 푸시하며(`moveStoryHere`), **옛 DB의 행은 지우지
  않는다** — 되돌릴 수 없는 쪽이 삭제다.
- 위치 판단 근거는 두 곳이다: 성공한 푸시를 적는 `twine-notion-pushed-db`와, pull이
  적는 장부(`twine-notion-synced-stories`). **둘을 섞지 않는다** — 후자는 "원격에서
  삭제됨" 판정에 쓰이는 물건이라 틀리면 스토리가 사라진다. 그래서 정보용 기록을 따로 뒀다.
  지금 DB는 `/__notion-sync/status`가 `dbId`로 내려준다.
