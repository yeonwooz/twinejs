...
홈(`/`)은 Twine의 스토리 카드 목록이 아니라 **인터랙티브 회고** 화면이다
(`src/routes/home/`). 화면 제목이 곧 이 앱의 이름이다.
위는 작문대, 아래는 연대기다.
...
- **스토리마다 저장 위치를 그 줄에서 고른다**(`story-db-button.tsx`). 버튼 라벨이 곧 지금
  어느 DB에 있는지이고, 누르면 워크스페이스의 stories DB 목록이 뜬다. **버튼은 언제나
  있다** — 고를 DB가 하나뿐이면 숨기게 뒀다가, 그게 저장 위치를 어디에도 안 보이게 만들고
  두 번째 DB를 만들 길까지 없애 기능 자체를 찾을 수 없었다. 고를 게 없으면
  `저장 위치 없음`/`노션 연결 안 됨`이라고 말하고, 메뉴에는 설정으로 가는 문
  (`저장 위치 추가·변경…`)이 항상 있다. 자세한 건 아래 "저장 위치는 스토리마다" 참고.
...
- **기본 저장 위치를 바꾸기 직전에 기존 스토리를 지금 DB에 못 박는다**
  ...
  사라진 것처럼 보인다.
- 노션·네트워크 오류는 `humanizeError`(`src/util/json-fetch.ts`)를 거쳐 화면에 나간다.
  원문을 그대로 내보냈더니 JSON 덩어리가 목록 한 줄을 무너뜨렸고, node undici의
  `fetch failed` 네 글자가 설정 화면에 그대로 찍혔다. `fetchJson`과 sync 양쪽이 같은
  함수를 쓴다.
- **"못 불러옴"과 "미연결"을 구별한다.** 네트워크가 한 번 끊기면 `useStoriesDbs`가
  `failed`를 세우고, 줄의 버튼은 `저장 위치 확인 안 됨`이라고 말하며 메뉴에
  `다시 불러오기`를 연다. 예전엔 실패를 `connected:false`로 뭉갰고 **그 실패를
  캐시까지 해서**, 로드 때 한 번 끊긴 것만으로 새로고침 전까지 모든 줄이
  `노션 연결 안 됨`에 갇혔다 — 헤더는 `노션에 저장됨`인데(sync 상태는 세션만 보므로
  네트워크와 무관하다) 줄마다는 끊겼다고 하는 꼴이었다. 실패는 캐시하지 않는다.
...
- **노션 id는 두 형태가 섞인다** — API가 주는 대시 있는 형태와, URL에서 뽑아
  `.env.local`에 적는 대시 없는 형태. `dbs.ts`의 GET이 `selected`를 **목록에 실제로 있는
  값으로 되맞춰** 돌려준다(`toNotionId`로 비교). 안 그러면 분명히 정해져 있는데도 라디오가
  하나도 안 켜져서 "아무것도 안 골랐다"로 보인다. 실제로 겪었다. 목록에 없는 id는 그대로
  둔다 — 엉뚱한 곳을 가리키고 있다는 사실 자체가 정보다.
