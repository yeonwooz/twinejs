...
import {currentDb, hasRoot} from '../../_lib/stories-db';

// GET /__notion-sync/stories (rewrite) → 이 사용자 stories DB의 RemoteStory[].
// ?meta=1 이면 twee 본문 없이 타임스탬프만 — 클라가 주기적으로 찔러보는 쪽.
//
// 읽는 DB는 고른 루트 페이지 아래 하나다. 각 행에 dbId를 붙여 보내는데, 클라가
// "원격에서 삭제됨" 판정을 이번에 실제로 읽은 DB로 한정하는 데 쓴다 — 저장 위치를
// 바꾼 직후 예전 DB의 스토리가 로컬에서 지워지는 것을 막는 장치다.
export default async function handler(req: VercelRequest, res: VercelResponse) {
	const s = getSession(req);

	if (!s?.token || !hasRoot(s)) {
		res.status(200).json([]);
		return;
	}

	try {
		// 값싼 폴링(?meta=1)에서는 노션을 뒤져 DB를 찾지 않는다 — 캐시된 dbId가 루트와
		// 맞을 때만 읽는다. 찾는 일은 앱 로드 때의 전체 목록 요청이 한다.
		if (req.query.meta && !(s.dbId && s.dbRootId === s.rootId)) {
			res.status(200).json([]);
			return;
		}
