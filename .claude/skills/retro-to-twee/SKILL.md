...
- **소스**: 루트 페이지(env `NOTION_RETRO_ROOT_PAGE_ID`) 아래
  ...
  └─ "N주차 회고"  (child_page, 여러 개면 주차 숫자 최대 = 최신)
        → 페이지 본문에 회고 내용을 직접 작성 (별도 하위 페이지 불필요)
  ```
  번역 소스 = "N주차 회고" 페이지 **본문**(하위 페이지로는 파고들지 않음).
  예전처럼 "회고 초안" 하위 페이지가 있으면 그걸 우선 사용한다.
...
### 1. 어느 주차인지 정하기
루트 페이지의 child_page에서 "N주차 회고" 목록을 읽고 **어느 주차를 작업할지 물어본다**
(자동 경로 `npm start`는 항상 물어봄; 기본 추천 = 주차 숫자 최대).

### 2. 회고 읽기
고른 "N주차 회고" 페이지의 **본문** 블록을 읽는다(`rich_text[].plain_text` 이어 붙이기).
**하위 페이지/DB(child_page/child_database)로는 파고들지 않는다.** 본문이 비어 있고 "회고 초안"
하위 페이지가 있으면 그 페이지를 대신 읽는다.

### 3. 비어있는 조건 채우기 (중요)
회고의 갈림길 중 **결과/후속이 안 적힌 분기**가 있으면 지어내지 말고 **사용자에게 물어본다.**
답을 받으면 twee에 반영하고 **원본 회고(주차 페이지 본문)도 완전한 내용으로 업데이트**한다
(이때 하위 페이지/DB는 보존하고 텍스트 블록만 교체).
